...
- Azure credentials configured (run `az login` before using Pulumi commands)
...
2. **Run the setup script** to configure your resource group and subscription:
   ```bash
   ./setup-config.sh
   ```
   
   Or manually set the required configuration:
   ```bash
   pulumi config set resourceGroupName <your-resource-group-name>
   pulumi config set subscriptionId <your-azure-subscription-id>
   ```

3. **Run a Pulumi preview** to see the import plan:
   ```bash
   pulumi preview
   ```
